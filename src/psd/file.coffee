...
module.exports = class File
  ...
  for own format, info of FORMATS then do (format, info) =>
    @::["read#{format}"] = -> @readf(info.code, info.length)
  ...
  readf: (format, len = null) ->
    value = jspack.Unpack format, @read(len or jspack.CalcLength(format))
    value = value[0]
    if format == '>q'
      value = new Long value[0], value[1], value[2]
      if value.getNumBitsAbs() > 32
